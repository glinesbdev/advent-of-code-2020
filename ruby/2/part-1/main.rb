...
DATA.each_line do |line|
  parts = line.gsub("'", '').gsub(':', '').split(' ')
  min, max, char, pass = [parts[0].split('-')[0], parts[0].split('-')[1], parts[1], parts[2]]
  occurrences = pass.scan(char)

  valid_passwords += 1 if occurrences.any? && occurrences.size.between?(min.to_i, max.to_i)
end
